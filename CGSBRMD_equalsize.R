#################################################################################
# CGSBRMD_equalsize:Circular generalized Strongly Balance Repeated Measure Design 
...
CGSBRMD_equalsize<-function(p,i,D=1){
...
minimal CGSBRMDs for", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    row <- paste(rep("=", 51), collapse = "")
    cat(row, "\n")
    print(object$S[[1]])
  })
...
# example#1
(H<-CGSBRMD_equalsize(p=3,i=3,D=1))
(D<-design_CGSBRMD(H))

# example #2
(H<-CGSBRMD_equalsize(p=5,i=3,D=2))
design_CGSBRMD(H)
